...
func TestCreateShortURLHandler(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			_, err := io.ReadAll(res.Body) // _ - resBody // todo get id and then new request
			defer res.Body.Close()         // TODO AHTUNG Defer AFTER READALL!
...
func TestGetLongURLHandler(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			_, err := io.ReadAll(res.Body) // _ - resBody //
